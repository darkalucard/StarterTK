...
int itemdb_read(const char *itemdb_file) {
	...
	memset(&item,0,sizeof(item));
		...
		if(SQL_ERROR==SqlStmt_Prepare(stmt,"SELECT `ItmId`, `ItmDescription`, `ItmIdentifier`, `ItmMark`, `ItmType`, `ItmBuyPrice`, `ItmSellPrice`, `ItmMaximumAmount`, `ItmPthId`, `ItmSex`, `ItmLevel`, `ItmLook`, "
			...
			"`ItmMaximumDamage`, `ItmText`, `ItmBuyText`, `ItmExchangeable`, `ItmDepositable`, `ItmDroppable`, `ItmWisdom`, `ItmCon`, `ItmAttackSpeed`, `ItmEvent`, `ItmVitaRequired`, `ItmManaRequired`, `ItmThrown`, "
			"`ItmUniqueAmount`, `ItmIndestructible`, `ItmTimer` FROM `Items`")
		...
		|| SQL_ERROR==SqlStmt_BindColumn(stmt,13,SQLDT_USHORT,&item.icon,0,NULL,NULL)
